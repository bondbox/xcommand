...
flake8:
	flake8 xkits_command xargproject --count --select=E9,F63,F7,F82 --show-source --statistics
	flake8 xkits_command xargproject --count --exit-zero --max-complexity=20 --max-line-length=127 --statistics --ignore=H405
pytest:
	pytest --cov=xkits_command --cov=xargproject --cov-report=term-missing --cov-report=xml --cov-report=html --cov-config=.coveragerc --cov-fail-under=100
pytest-clean:
	rm -rf .pytest_cache
